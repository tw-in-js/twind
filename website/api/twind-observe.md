...
head:
  - - meta
    - name: description
      content: API for the twind/observe module
  - - meta
    - name: keywords
...
`twind/observe` uses the {@link twind.tw default/global `tw`} instance if not configured otherwise. You can provide a custom instance in several ways:
