...
test('class names are hashed', () => {
  assert.strictEqual(tw.inject('flex text-center'), '#e9txhd #1vaw68m')
  assert.deepEqual(tw.target, ['.\\#1vaw68m{text-align:center}', '.\\#e9txhd{display:flex}'])
})

test('accept already hashed rules', () => {
  assert.strictEqual(tw.inject('#1bk5mm5 #bibj42'), '#1bk5mm5 #bibj42')
  assert.deepEqual(tw.target, [])
})

test('different variant produce different hashes', () => {
  assert.strictEqual(tw.inject('sm:text-center lg:text-center'), '#7n36h2 #18xkmkh')
  assert.deepEqual(tw.target, [
...
test('same style in different layers has different hash', () => {
  assert.strictEqual(
    tw.inject(`w-0 ${css({ width: '0px' })} ${apply(`w-0`)}`),
